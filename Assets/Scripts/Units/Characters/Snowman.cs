    ...
    public class Snowman : Character
    {
        ...
        private void Awake()
        {
            foreach (var house in FindObjectsByType<House>(FindObjectsSortMode.None))
            {
                ...
                if (Vector2Int.Distance(GridPosition, house.GridPosition) < Vector2Int.Distance(_targetHouse.GridPosition, house.GridPosition))
                {
                    _targetHouse = house;
                }
            }
            ...
            { 
                ...
                if (tile && !tile.Unit)
                {
                    bestDir = Vector2Int.Distance(
                              ...
                              < Vector2Int.Distance(
                                  new(GridPosition.x + bestDir.x, GridPosition.y + bestDir.y), _targetHouse.GridPosition) 
                        ? offset 
                        : bestDir;
                }
            }
